...
Texture::Texture(const std::string &imagePath) : m_Id{}, m_Width{10.0f}, m_Height{10.0f}, m_CreationOk{false}
{
    CreateFromImage(imagePath);
}

Texture::Texture(const std::string &text, TTF_Font *pFont, const Color4f &textColor)
    : m_Id{}, m_Width{10.0f}, m_Height{10.0f}, m_CreationOk{false}
{
...
Texture::Texture(Texture &&other) noexcept
    : m_Id{other.m_Id}, m_Width{other.m_Width}, m_Height{other.m_Height}, m_CreationOk{other.m_CreationOk}
{
    other.m_Id = 0;
...
void Texture::CreateFromString(const std::string &text, const std::string &fontPath, int ptSize,
...
void Texture::CreateFromString(const std::string &text, TTF_Font *pFont, const Color4f &color)
{
    ...
    if (pLoadedSurface == nullptr)
    {
        std::cerr << "Texture::CreateFromString, error when calling TTF_RenderText_Blended: " << TTF_GetError()
                  << std::endl;
        m_CreationOk = false;
        return;
    }
...
void Texture::CreateFromSurface(SDL_Surface *pSurface)
{
    ...
    case 4:
        ...
        break;
    default:
        std::cerr << "Texture::CreateFromSurface, unknown pixel format, BytesPerPixel: "
                  << static_cast<int>(pSurface->format->BytesPerPixel) << "\nUse 32 bit or 24 bit images.\n";
        m_CreationOk = false;
        return;
    }
    ...
    // check for errors. Can happen if a texture is created while a static pointer is being initialized, even before the
    // call to the main function.
    GLenum e = glGetError();
    if (e != GL_NO_ERROR)
    {
        std::cerr << "Texture::CreateFromSurface, error binding textures, Error id = " << e << '\n';
        std::cerr << "Can happen if a texture is created before performing the initialization code (e.g. a static "
                     "Texture object).\n";
    ...
    //                0:    The mipmap level.  0, since we want to update the base level mipmap image (i.e., the image
    //                itself,
    ...
    //                         getting one byte.  This is fairly typical--it means that the image can store, for each
    //                         channel, any value that fits in one byte (so 0 through 255).  These values are to be
    //                         interpreted as *unsigned* values (since 0x00 should be dark and 0xFF should be bright).
    //  surface->pixels:    The actual data.  As above, SDL's array of bytes.
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, pSurface->w, pSurface->h, 0, pixelFormat, GL_UNSIGNED_BYTE,
                 pSurface->pixels);

    // Set the minification and magnification filters.  In this case, when the texture is minified (i.e., the texture's
    // pixels (texels) are *smaller* than the screen pixels you're seeing them on, linearly filter them (i.e. blend them
    // together).  This blends four texels for each sample--which is not very much.  Mipmapping can give better results.
    // Find a texturing tutorial that discusses these issues further.  Conversely, when the texture is magnified (i.e.,
    // the texture's texels are *larger* than the screen pixels you're seeing them on), linearly filter them.
    // Qualitatively, this causes "blown up" (overmagnified) textures to look blurry instead of blocky.
