...
void utils::DrawArc(float centerX, float centerY, float radX, float radY, float fromAngle, float tillAngle,
                    float lineWidth)
{
...
bool utils::IsOverlapping(const Vector2f &a, const Vector2f &b, const Rectf &r)
{
    ...
    HitInfo hitInfo{};
    Vector2f vertices[]{Vector2f{r.left, r.bottom}, Vector2f{r.left + r.width, r.bottom},
                        Vector2f{r.left + r.width, r.bottom + r.height}, Vector2f{r.left, r.bottom + r.height}};
...
bool utils::IsOverlapping(const Rectf &r, const Circlef &c)
{
    ...
    // Check line segments
    if (utils::DistPointLineSegment(c.center, Vector2f{r.left, r.bottom}, Vector2f{r.left, r.bottom + r.height}) <=
        c.radius)
    {
        return true;
    }
    if (utils::DistPointLineSegment(c.center, Vector2f{r.left, r.bottom}, Vector2f{r.left + r.width, r.bottom}) <=
        c.radius)
    {
        return true;
    }
    if (utils::DistPointLineSegment(c.center, Vector2f{r.left + r.width, r.bottom + r.height},
                                    Vector2f{r.left, r.bottom + r.height}) <= c.radius)
    {
        return true;
    }
    if (utils::DistPointLineSegment(c.center, Vector2f{r.left + r.width, r.bottom + r.height},
                                    Vector2f{r.left + r.width, r.bottom}) <= c.radius)
    {
        return true;
    }
    return false;
...
bool utils::IntersectLineSegments(const Vector2f &p1, const Vector2f &p2, const Vector2f &q1, const Vector2f &q2,
                                  float &outLambda1, float &outLambda2, float epsilon)
{
    ...
    else // are parallel
    {
        ...
        outLambda1 = 0;
        outLambda2 = 0;
        if (utils::IsPointOnLineSegment(p1, q1, q2) || utils::IsPointOnLineSegment(p2, q1, q2) ||
            utils::IsPointOnLineSegment(q1, p1, p2) || utils::IsPointOnLineSegment(q2, p1, p2))
        {
            intersecting = true;
        }
    }
...
bool utils::Raycast(const std::vector<Vector2f> &vertices, const Vector2f &rayP1, const Vector2f &rayP2,
                    HitInfo &hitInfo)
{
    return Raycast(vertices.data(), vertices.size(), rayP1, rayP2, hitInfo);
}

bool utils::Raycast(const Vector2f *vertices, const size_t nrVertices, const Vector2f &rayP1, const Vector2f &rayP2,
                    HitInfo &hitInfo)
{
    if (nrVertices == 0)
    {
        ...
        if (IsOverlapping(r1, r2))
        {
            ...
            if (IntersectLineSegments(rayP1, rayP2, q1, q2, lambda1, lambda2))
            {
                if (lambda1 > 0 && lambda1 <= 1 && lambda2 > 0 && lambda2 <= 1)
                {
                    ...
                    linesHitInfo.intersectPoint =
                        Vector2f{rayP1.x + ((rayP2.x - rayP1.x) * lambda1), rayP1.y + ((rayP2.y - rayP1.y) * lambda1)};
    ...
    hitInfo = *std::min_element(hits.begin(), hits.end(),
                                [](const HitInfo &first, const HitInfo &last) { return first.lambda < last.lambda; });
    return true;
}
...
bool utils::IntersectRectLine(const Rectf &r, const Vector2f &p1, const Vector2f &p2, float &intersectMin,
                              float &intersectMax)
{
    // Parameters
    // input:
...
float utils::SineBetween(float x, float cycleDuration, float min, float max)
{
    const float halfLength{(max - min) * 0.5f};
    const float stretch{(2.f * g_Pi) / cycleDuration};
