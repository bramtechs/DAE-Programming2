...
I - Insert hitbox point.
X - Delete hitbox point.
DELETE - Destroy hitbox polygon.
PAGE UP/PAGE DOWN - Change snap grid size.
RETURN - Save hitboxes.
)");
}
...
void Editor::Update(Camera &camera)
{
    ...
    {
        const Vector2f snappedCursor{utils::SnapRounded(m_CursorPos.x, m_GridScale),
...
void Level::AddColliderAround(const Vector2f &center, float radius)
{
    PolygonCollider collider{};
    collider.AddPoint(center.x - radius * 0.5f, center.y - radius * 0.5f);
    collider.AddPoint(center.x + radius * 0.5f, center.y - radius * 0.5f);
    collider.AddPoint(center.x + radius * 0.5f, center.y + radius * 0.5f);
    collider.AddPoint(center.x - radius * 0.5f, center.y + radius * 0.5f);
...
void Level::DeleteCollider(int index)
{
    if (index >= 0 && index < m_Colliders.size())
    {
        m_Colliders.erase(std::next(m_Colliders.cbegin(), index));
    }
}
...
void Editor::ProcessKeyDownEvent(const SDL_KeyboardEvent &e)
{
    ...
    {
        m_pLevel->AddColliderAround(m_CursorPos, 2);
    }
    else
    {
        std::vector<PolygonCollider> &colliders{m_pLevel->GetColliders()};
        for (int i{}; i < colliders.size(); ++i)
        {
            PolygonCollider::Signal signal{colliders[i].ProcessKeyDownEvent(e)};
            if (signal != PolygonCollider::Signal::none)
            {
                if (signal == PolygonCollider::Signal::destroy)
                {
                    m_pLevel->DeleteCollider(i);
                }
                break;
            }
        }
