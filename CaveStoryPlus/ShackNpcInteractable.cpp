...
bool ShackNpcInteractable::OnInteract(Game &game)
{
    ...
    DialogMessage &msg{man.QueueMessage({"The Doctor--he killed my older brother."})};

    if (game.GetPlayer()->DidCollectPickup())
    {
    ...
    {
        msg.SetReadEvent(new SpawnBossEnemyDialogEvent(*this));
    }
...
{
    const Vector2f bossSpawnPos{5.f, 6.72f};
    const float bossAreaLeft{5.f};
    const float bossAreaRight{17.f};
    BossEnemy *pBoss{new BossEnemy(bossSpawnPos, bossAreaLeft, bossAreaRight)};
